...
/// ImageLoader 处理二进制文件的时机是在 main 入口函数以前，它在加载文件时主要做两个工作：在程序运行时它先将动态链接的 image 递归加载 （也就是上面测试栈中一串的递归调用的时刻） 再从可执行文件 image 递归加载所有符号
void
load_images(const char *path __unused, const struct mach_header *mh)
{
    ...
    // Discover load methods
    {
        ...
        /// 为加载load方法前期作准备 class 和 categories
...
{
    /// 为了查询 load 函数列表，会分别查询该函数在内存数据段上指定 section 区域是否有所记录。
    size_t count;
                       ...
                       bool initialize, bool cache, bool resolver)
{
    ...
    /// 调用 initalize 类方法  根据元类的 databit -> flag 标志存储位置
